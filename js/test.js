...
function renderCurrentQuestion() {
    ...
    if (photoImg) photoImg.src = question.photoUrl;
    if (optionsContainer) {
...
function handleAnswer(selectedIdx) {
    ...
    // Визуально обновляем прогресс (закрашиваем текущий сегмент)
    renderQuizProgress();
    
    // Блокируем кнопки на время паузы
    const btns = document.querySelectorAll('.quiz-option-btn');
    btns.forEach(btn => btn.classList.add('disabled'));
    
    // Переход к следующему вопросу через короткую паузу
    setTimeout(() => {
        quizState.currentIndex++;
        if (quizState.currentIndex < quizState.totalQuestions) {
            renderCurrentQuestion();
            // Разблокируем кнопки
            document.querySelectorAll('.quiz-option-btn').forEach(btn => btn.classList.remove('disabled'));
        } else {
            finishQuiz();
        }
    }, 400);
